...
def connect_to_dremio_flight_server_endpoint_using_polars(host, port, username, password, query,
                                             tls, certs, disable_server_verification, pat_or_auth_token,
                                             engine, session_properties, output_file):
    """
    Connects to Dremio Flight server endpoint with the provided credentials.
    ...
    try:
        ...
        if query:
            ...
            if output_file is not None:
                ...
                res_table=(reader.read_all())
                print('client_obj')
                print(res_table.schema)
                py_schema = {
                    'LastLoginDateTime': 'timestamp[ms]',
                    'Latitude': 'double',
                    'Longitude': 'double'
                }
                # Constructing a DataFrame from an Arrow Table:
                result = pl.from_arrow(res_table)
                return result
